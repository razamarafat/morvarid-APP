                        ...
                        <div key={farm.id} className="group shadow-sm hover:shadow-xl transition-all duration-300 rounded-[28px] overflow-hidden border border-gray-200 dark:border-gray-700">
                            <div onClick={() => toggleFarm(farm.id)} className={`p-5 lg:p-7 flex items-center justify-between cursor-pointer transition-colors border-r-[8px] ${hasStats ? 'bg-white dark:bg-gray-800 border-green-500 hover:bg-gray-50 dark:hover:bg-gray-700/50' : 'bg-white dark:bg-gray-800 border-red-500 hover:bg-red-50 dark:hover:bg-red-900/10'}`}>
                                ...
                                <div className="flex items-center gap-4 lg:gap-8">
                                    ...
                                    {!hasStats && (
                                        <Button size="sm" variant="danger" onClick={(e) => handleSendAlert(farm.id, farm.name, e)} isLoading={alertLoading === farm.id} className="font-black lg:text-lg lg:h-12 lg:px-6 rounded-full">
                            ...
                            <AnimatePresence>
                                {isExpanded && hasStats && (
                                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="bg-gray-50 dark:bg-black/20 p-5 lg:p-8 border-t border-gray-200 dark:border-gray-700">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-8">
                                            {sortedFarmStats.map(stat => {
                                                ...
                                                const relevantInvoices = invoices.filter(inv => inv.farmId === farm.id && inv.productId === stat.productId && normalizeDate(inv.date) === normalizeDate(stat.date));
                                                ...
                                                return (
                                                    <div key={stat.id} className="bg-white dark:bg-gray-700 rounded-[24px] p-5 lg:p-6 shadow-sm border border-gray-200 dark:border-gray-600 relative overflow-hidden transition-shadow hover:shadow-md">
                                                        ...
                                                        <div className="mb-6 relative h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                                                            <div className="absolute top-0 right-0 h-full bg-metro-orange rounded-full transition-all duration-1000 ease-out" style={{ width: `${soldPercent}%` }}></div>
                                                        </div>
...
const InvoiceList = () => {
    ...
    const filteredInvoices = invoices.filter(i => i.date === normalizedFilterDate && (selectedFarmId === 'all' || i.farmId === selectedFarmId)).sort((a, b) => b.createdAt - a.createdAt); 
        ...
        <div className="space-y-6 lg:space-y-8">
             <div className="bg-white dark:bg-gray-800 p-6 lg:p-8 shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col md:flex-row gap-6 items-end rounded-[28px]">
                <div className="w-full flex-1">
                    ...
                    <select className="w-full p-3 lg:p-4 border-2 border-gray-200 dark:border-gray-700 bg-white text-gray-900 dark:bg-gray-800 dark:text-white font-bold outline-none focus:border-metro-orange rounded-xl text-base lg:text-xl transition-colors" value={selectedFarmId} onChange={(e) => setSelectedFarmId(e.target.value)}>
                    ...
                    filteredInvoices.map(i => (
                        <div key={i.id} className="bg-white dark:bg-gray-800 p-5 rounded-[24px] shadow-sm border-l-8 border-metro-orange">
                            <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-100 dark:border-gray-700">
                                <span className="text-xs font-bold text-gray-400">رمز حواله</span>
                                <div className="flex gap-1" dir="ltr">{renderInvoiceNumber(i.invoiceNumber)}</div>
                            </div>
                            <div className="grid grid-cols-2 gap-4 mb-4">
                                <div><span className="text-xs font-bold text-gray-400 block mb-1">فارم</span><span className="font-bold text-lg dark:text-white">{farms.find(f => f.id === i.farmId)?.name}</span></div>
                                <div><span className="text-xs font-bold text-gray-400 block mb-1">ساعت</span><span className="font-mono font-bold text-lg dark:text-gray-300">{toPersianDigits(new Date(i.createdAt).toLocaleTimeString('fa-IR', {hour: '2-digit', minute:'2-digit'}))}</span></div>
                            </div>
                            <div className="grid grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-700/50 p-3 rounded-xl mb-4">
                                <div className="text-center"><span className="block text-[10px] font-bold text-gray-500 mb-1">تعداد</span><span className="font-black text-xl text-gray-800 dark:text-white">{toPersianDigits(i.totalCartons)}</span></div>
                                <div className="text-center border-r border-gray-200 dark:border-gray-600"><span className="block text-[10px] font-bold text-gray-500 mb-1">وزن</span><span className="font-black text-xl text-metro-blue">{toPersianDigits(i.totalWeight)}</span></div>
                            </div>
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-2"><div className="w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-500"><Icons.User className="w-4 h-4" /></div><span className="text-sm font-bold text-gray-600 dark:text-gray-300">{(i as any).creatorName}</span></div>
                                <span className={`px-3 py-1 text-xs font-black text-white rounded-full ${i.updatedAt ? 'bg-amber-500' : 'bg-metro-green'}`}>{i.updatedAt ? 'ویرایش شده' : 'عادی'}</span>
                            </div>
                        </div>
                    ))
                )}
                            ...
                            {filteredInvoices.length === 0 ? (
                                ...
                                filteredInvoices.map(i => (
                                    <tr key={i.id} className="hover:bg-blue-50 dark:hover:bg-gray-700/50 transition-colors text-gray-800 dark:text-gray-200">
                                        <td className="px-6 py-5 text-center dir-ltr" dir="ltr">{renderInvoiceNumber(i.invoiceNumber)}</td>
                                        ...
                                        <td className="px-6 py-5"><span className={`px-4 py-1.5 text-sm lg:text-base font-black text-white w-fit rounded-full ${i.updatedAt ? 'bg-amber-500' : 'bg-metro-green'}`}>{i.updatedAt ? 'ویرایش شده' : 'عادی'}</span></td>
                                        <td className="px-6 py-5 whitespace-nowrap text-base lg:text-lg font-black text-gray-700 dark:text-gray-200">{(i as any).creatorName}</td>
                                        <td className="px-6 py-5 text-center whitespace-nowrap font-mono font-bold text-base lg:text-lg text-gray-600 dark:text-gray-400">{toPersianDigits(new Date(i.createdAt).toLocaleTimeString('fa-IR', {hour: '2-digit', minute:'2-digit'}))}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
    ...
    return (
        <div className="space-y-6 lg:space-y-8">
            <div className="bg-white dark:bg-gray-800 p-6 lg:p-8 rounded-[28px] shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row gap-6 justify-between items-center">
                <div className="flex items-center gap-3"><div className="p-3 bg-purple-100 dark:bg-purple-900/30 rounded-2xl text-purple-600"><Icons.BarChart className="w-6 h-6 lg:w-8 lg:h-8" /></div><h3 className="font-black text-xl lg:text-3xl dark:text-white">تحلیل تولید و فروش</h3></div>
                <div className="flex gap-3 w-full sm:w-auto">
                    <select value={selectedFarmId} onChange={(e) => setSelectedFarmId(e.target.value)} className="p-3 lg:p-4 border-2 border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 dark:text-white font-bold text-sm lg:text-lg w-full sm:w-48 outline-none focus:border-purple-500 transition-colors">
                        <option value="all">همه فارم‌ها</option>
                        {farms.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}
                    </select>
                    <select value={range} onChange={(e) => setRange(Number(e.target.value))} className="p-3 lg:p-4 border-2 border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 dark:text-white font-bold text-sm lg:text-lg outline-none focus:border-purple-500 transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-8">
                <div className="bg-green-50 dark:bg-green-900/20 p-6 lg:p-8 rounded-[28px] border border-green-100 dark:border-green-800 flex items-center justify-between shadow-sm"><div><span className="text-sm lg:text-base font-bold text-green-700 dark:text-green-300 block mb-2">مجموع تولید دوره</span><span className="text-3xl lg:text-5xl font-black text-green-800 dark:text-green-100">{toPersianDigits(chartData.totalProd)}</span></div><div className="p-4 bg-white dark:bg-green-800/30 rounded-2xl"><Icons.BarChart className="w-8 h-8 lg:w-10 lg:h-10 text-green-500" /></div></div>
                <div className="bg-red-50 dark:bg-red-900/20 p-6 lg:p-8 rounded-[28px] border border-red-100 dark:border-red-800 flex items-center justify-between shadow-sm"><div><span className="text-sm lg:text-base font-bold text-red-700 dark:text-red-300 block mb-2">مجموع فروش دوره</span><span className="text-3xl lg:text-5xl font-black text-red-800 dark:text-red-100">{toPersianDigits(chartData.totalSales)}</span></div><div className="p-4 bg-white dark:bg-red-800/30 rounded-2xl"><Icons.FileText className="w-8 h-8 lg:w-10 lg:h-10 text-red-500" /></div></div>
                <div className="bg-blue-50 dark:bg-blue-900/20 p-6 lg:p-8 rounded-[28px] border border-blue-100 dark:border-blue-800 flex items-center justify-between shadow-sm"><div><span className="text-sm lg:text-base font-bold text-blue-700 dark:text-blue-300 block mb-2">نرخ فروش به تولید</span><span className="text-3xl lg:text-5xl font-black text-blue-800 dark:text-blue-100">٪ {toPersianDigits(salesRate)}</span></div><div className="p-4 bg-white dark:bg-blue-800/30 rounded-2xl"><Icons.Refresh className="w-8 h-8 lg:w-10 lg:h-10 text-blue-500" /></div></div>
            </div>
                 ...
                 <div className="min-w-[700px] h-[350px] lg:h-[450px] relative pt-6 pb-8 px-4 flex items-end gap-4 lg:gap-6">
                    <div className="absolute inset-0 flex flex-col justify-between pointer-events-none pb-8 pt-6 px-4">
                        {[1, 0.75, 0.5, 0.25, 0].map((tick, i) => (
                            <div key={i} className="w-full border-b border-gray-100 dark:border-gray-700 h-0 relative"><span className="absolute -left-8 -top-2 text-sm lg:text-base text-gray-400 font-mono">{toPersianDigits(Math.round(chartData.maxVal * tick))}</span></div>
                        ))}
                    </div>
                    {chartData.dataPoints.length === 0 ? <div className="absolute inset-0 flex items-center justify-center text-gray-400 lg:text-2xl font-bold">داده‌ای برای نمایش وجود ندارد</div> : chartData.dataPoints.map((point, index) => {
                        ...
                        return (
                            <div key={index} className="flex-1 flex flex-col justify-end items-center gap-1 group relative h-full z-10">
                                <div className="absolute bottom-full mb-3 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-900 text-white text-xs lg:text-base p-3 rounded-lg pointer-events-none whitespace-nowrap z-20 shadow-xl"><div className="text-center font-bold mb-1 border-b border-white/20 pb-1">{point.date}</div><div>تولید: {toPersianDigits(point.prod)}</div><div>فروش: {toPersianDigits(point.sale)}</div></div>
                                <div className="w-full flex justify-center items-end gap-1.5 h-full px-1">
                                    <motion.div initial={{ height: 0 }} animate={{ height: `${prodHeight}%` }} transition={{ duration: 0.5, delay: index * 0.05 }} className="w-full max-w-[24px] lg:max-w-[48px] bg-green-500 rounded-t-md hover:bg-green-400 transition-colors relative shadow-sm">{prodHeight > 10 && <span className="absolute top-1 left-0 w-full text-center text-[10px] lg:text-xs text-white/90 font-bold">{toPersianDigits(point.prod)}</span>}</motion.div>
                                    <motion.div initial={{ height: 0 }} animate={{ height: `${saleHeight}%` }} transition={{ duration: 0.5, delay: index * 0.05 + 0.1 }} className="w-full max-w-[24px] lg:max-w-[48px] bg-red-500 rounded-t-md hover:bg-red-400 transition-colors relative shadow-sm">{saleHeight > 10 && <span className="absolute top-1 left-0 w-full text-center text-[10px] lg:text-xs text-white/90 font-bold">{toPersianDigits(point.sale)}</span>}</motion.div>
                                </div>
                                <div className="h-8 flex items-center justify-center"><span className="text-xs lg:text-base text-gray-500 dark:text-gray-400 -rotate-45 whitespace-nowrap origin-top-left translate-y-3 translate-x-2 font-medium">{toPersianDigits(point.date.slice(5))}</span></div>
                            </div>
                        );
                    })}
                </div>
                <div className="flex justify-center gap-6 lg:gap-10 mt-6 lg:mt-10"><div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700 px-4 py-2 rounded-full"><div className="w-3 h-3 lg:w-4 lg:h-4 bg-green-500 rounded-full"></div><span className="text-sm lg:text-lg font-bold text-gray-600 dark:text-gray-300">تولید روزانه</span></div><div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700 px-4 py-2 rounded-full"><div className="w-3 h-3 lg:w-4 lg:h-4 bg-red-50 rounded-full"></div><span className="text-sm lg:text-lg font-bold text-gray-600 dark:text-gray-300">فروش روزانه</span></div></div>
            </div>
        </div>
    );
};
