...
export const useUserStore = create<UserState>((set, get) => ({
  ...
  addUser: async (userData) => {
      ...
      // Never use a hardcoded default password like '1234'.
      // If admin provides a password, use it. Otherwise, generate a secure random one.
      const isAutoGenerated = !userData.password;
      const password = userData.password || generateSecurePassword(16); // 16 chars for better security
      ...
      try {
          ...
          // 🚨 SECURITY WARNING: ROLE ASSIGNMENT 🚨
          // Currently, 'role' is passed via client-side metadata (options.data).
          // This allows the DB trigger (handle_new_user) to populate 'public.profiles'.
          // RISK: A malicious user could potentially call signUp with a modified role.
          // MITIGATION (TODO): Move user creation to a Supabase Edge Function (Server-Side Admin Context)
          // to verify admin privileges before assigning roles.
          const { data, error } = await (tempSupabase.auth as any).signUp({
              ...
              options: { 
                  data: { 
                      ...
                      role: userData.role // Passed to trigger
                  } 
              }
          });
          ...
          if (data.user) {
              // Note: The Trigger in DB usually handles profile creation.
              // We handle farm assignments manually here as triggers usually don't handle relational tables.
              ...
              // SEC-04: Return password to UI for the admin to copy, NEVER log it.
          ...
          console.error('User Add Exception:', err); // Log generic error, not credentials
