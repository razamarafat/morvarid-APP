...
export const useSalesVoucherStore = create<SalesVoucherState>((set, get) => ({
  ...
  createSalesVoucher: async (input: CreateSalesVoucherInput) => {
    ...
    try {
      // 1. Insert the voucher (voucher_number auto-generated by DB trigger)
      const { data: voucher, error: voucherError } = await supabase
        .from('sales_vouchers')
        .insert({
  ...
  // ============================
  // CANCEL VOUCHER (admin only, reverses inventory)
  // ============================
  cancelSalesVoucher: async (id: string) => {
    ...
    if (!currentUser || currentUser.role !== 'ADMIN') {
      set({ isSubmitting: false });
      return { success: false, error: 'فقط مدیر سیستم می‌تواند حواله را کنسل کند.' };
    }
  ...
  // ============================
  // DELETE VOUCHER (admin only, draft only)
  // ============================
  deleteSalesVoucher: async (id: string) => {
    ...
    if (!currentUser || currentUser.role !== 'ADMIN') {
      set({ isSubmitting: false });
      return { success: false, error: 'فقط مدیر سیستم می‌تواند حواله را حذف کند.' };
    }
  ...
  getNextVoucherNumber: async (): Promise<string> => {
    try {
      const { data } = await supabase
        .rpc('get_next_sales_voucher_number')
        .single();

      if (data) return data as string;
    } catch {
      // Fallback: use timestamp-based number
      console.warn('Could not get next voucher number from DB, using fallback');
    }
    // Fallback
    const count = get().vouchers.length + 1;
    return `SV-${String(count).padStart(4, '0')}`;
  },
