...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children, title, onNavigate, currentView }) => {
  ...
  // ----------------------------------------------------------------------------
  // PageHeader visibility: rendered for every sub-page (currentView set and
  // not equal to 'dashboard'). Hidden on the role's root dashboard home.
  // Lives inside <main> so the back button is in immediate visual context
  // of the page title (the placement that failed the previous UAT round).
  // ----------------------------------------------------------------------------
  const showPageHeader = !!currentView && currentView !== 'dashboard';
