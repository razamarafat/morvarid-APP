...
const FarmStatistics = () => {
    ...
    const handleSendAlert = (farmName: string) => {
        ...
        addToast(`هشدار ثبت آمار برای "${farmName}" ارسال شد`, 'success');
    };
        ...
        <div className="space-y-6">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-[24px] shadow-sm">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="font-bold text-lg dark:text-white">وضعیت آمار روزانه ({today})</h3>
                    <select 
                        className="p-2 border rounded-xl dark:bg-gray-700 dark:border-gray-600"
                ...
                <div className="grid gap-4">
                    {filteredFarms.map(farm => {
                        ...
                        const hasStatsToday = statistics.some(s => s.farmId === farm.id && s.date === today);
                        
                        return (
                            <div key={farm.id} className={`p-4 rounded-2xl border ${hasStatsToday ? 'border-green-200 bg-green-50 dark:bg-green-900/20' : 'border-red-200 bg-red-50 dark:bg-red-900/20'} flex justify-between items-center`}>
                                <div>
                                    <h4 className="font-bold text-gray-800 dark:text-white">{farm.name}</h4>
                                    <span className={`text-xs ${hasStatsToday ? 'text-green-600' : 'text-red-600'}`}>
                                        {hasStatsToday ? 'آمار امروز ثبت شده' : 'هنوز ثبت نشده'}
                                    </span>
                                </div>
                                {hasStatsToday ? (
                                    <Button size="sm" variant="secondary" onClick={() => setSelectedFarmId(farm.id)}>مشاهده جزئیات</Button>
                                ) : (
                        ...
                        <tbody>
                            {statistics
                                ...
                                <tr key={s.id} className="bg-white dark:bg-gray-800 border-b dark:border-gray-700">
                                    ...
                                    <td className="px-6 py-4 text-red-600">{s.sales || '-'}</td>
                                    <td className="px-6 py-4 font-bold">{s.currentInventory || '-'}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
...
const InvoiceList = () => {
    ...
    return (
        <div className="space-y-6">
             <div className="bg-white dark:bg-gray-800 p-6 rounded-[24px] shadow-sm flex justify-between items-center">
                <h3 className="font-bold text-lg dark:text-white">حواله‌های فروش</h3>
                <select 
                    className="p-2 border rounded-xl dark:bg-gray-700 dark:border-gray-600"
                    value={selectedFarmId}
                    onChange={(e) => setSelectedFarmId(e.target.value)}
                            ...
                            {filteredInvoices.map(i => (
                                <tr key={i.id} className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                    ...
                                    <td className="px-6 py-4">
                                        {i.isYesterday ? (
                                            <span className="bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded-full">دیروزی</span>
                                        ) : (
                                            <span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full">عادی</span>
                                        )}
                                    </td>
                                </tr>
                            ))}
                             {filteredInvoices.length === 0 && <tr><td colSpan={6} className="text-center py-8">حواله‌ای یافت نشد</td></tr>}
                        </tbody>
                    </table>
                </div>
            </div>
